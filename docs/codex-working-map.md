...
- 影月议会材质主体必须是黑紫膜包裹的黏稠压力核心，传输会鼓胀与收束，恶魔裂隙有内陷空间膜，疯狂广播是掠过棋盘的低矮压力前沿；灵魂使用蓝紫流体，诅咒与低语使用暗红紫。酸绿直线、空心圆和全卡绿色闪烁只允许作次级读数，不能重新承担法术主体。持续状态仍由 `CardStatusOverlay` 消费真实状态并保持15 FPS低频呼吸，不能为了增强表现重复触发规则、叠加疯狂或在每张卡每帧重建数百个 Ribbon 面片。共享 Ribbon 必须过滤非有限、重复和子像素退化路径，使用直接三角形原语；调整后运行 `tools/test_vfx_canvas_toolkit.gd` 和影月专项测试。
...
- 狐妖仙表现优先读 `scripts/ui/animation/fox_spirit_animation_provider.gd`、`fox_spirit_target_visual.gd`、`fox_spirit_area_visual.gd`、`fox_spirit_ritual_visual.gd`、`scripts/ui/tail_resource_meter.gd` 和 `scripts/ui/card_status_overlay.gd`。Provider 只做路由、坐标、锚点、时序和回收；Target 负责献祭/复生授予/勾魄/两类控制/衍生牌入场，Area 只负责精确 2x2 狐火，Ritual 负责祸国/九尾军势/天狐化。狐尾、魂丝和控制权丝带使用有厚度的镜面妖气，目标周围的香雾膜与错相光点只建立纵深，不遮挡卡面；尾数仪表静态阶段仍保留低频错相呼吸。永久魅惑使用完整控制权边框，狐念之术保留原阵营底色与倒计时，禁止使用爱心表达。祸国的 `ruin_country_targets` 必须消费 `SacrificeFriendlyMinionsEffect` 已解析的真实目标矩形，只标记友方非英雄随从，不得用固定 5x5 图案伪造英雄、敌方或空格目标。`EvolveUnitsEffect.animation`、`GrantRebornEffect.animation`、`AddCardToHandEffect.animation` 与随从 `CardData.animation` 是通用表现钩子，不得在效果类或 `HandPlayResolver` 中按狐妖卡牌 id 分支。九尾用 `fox_reborn` 表现授予，其他来源的 `reborn` 不得被狐妖 provider 接管。普通献祭先指向可见尾数仪表，再由同一动画根节点回勾到场上苏妲己；九尾献祭使用独立月白复生反馈，不得伪造资源继续增长。修改后运行 `tools/test_fox_spirit_animation_provider.gd`、`tools/test_animation_routing.gd` 与 `tools/test_right_side_hud.gd`。
...
- 右侧 HUD 的面板外壳、标题、按钮、指标块与资源刻度统一复用 `RightSideHudStyle`；无专属贴图的稳定语义图标复用 `HudSymbolIcon`。法力、翻牌和资源分采用图标加短数值；通用小上限资源使用离散刻度。资源具有不可替代的形状语义时可提供只读专属仪表，但仍须嵌入统一 HUD 外壳、由 controller 传入规则值且不得反向修改状态；`TailResourceMeter` 是九尾扇形九槽的首个例子。修改后运行 `tools/test_right_side_hud.gd`。
