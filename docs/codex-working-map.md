...
- 共生体。优先读 `data/cards.json` 中的 `symbiote.faction_card_pools` / `tokens[]`、`scripts/game/symbiote_offspring_pool_resolver.gd`、`scripts/game/unit_silence_resolver.gd`、`scripts/effects/update_symbiote_offspring_pool_effect.gd`、`attach_symbiote_offspring_effect.gd`、`random_normal_attacks_effect.gd`、`symbiote_whip_effect.gd`、`symbiote_absorb_effect.gd`、`carnage_kill_progress_effect.gd`、`scripts/actions/attack_action.gd`、`scripts/game/death_resolver.gd`、`scripts/data/card_state.gd` 和 `scripts/ui/animation/symbiote_animation_provider.gd`。静态配置声明初始牌、解锁规则和 `with_replacement_card_ids`；玩家动态池仅保存在 `PlayerState.effect_runtime_values.symbiote_offspring_pool`，初始牌只初始化一次，后续查询绝不能补回不放回卡。解锁必须通过稳定 rule id 幂等执行，不能把池进度写回 `CardData` 或 UI。共生体组织只选择 `target_card_ids` 中的己方宿主；目标二次校验后才抽池，共生体战士保留，其余结果移除，然后使用无 transform 状态的 `transform_to_card_data()` 永久替换宿主并保留已消耗行动。`inherit_host_base_stats_card_ids` 声明基因战士等特殊宿主：替换前读取卡牌基础攻击/生命，替换后叠加并写入永久属性覆盖，不得用状态模拟。动画不得提前抽池。剥离链顺序为 `play_animation -> gain_permanent_attack -> update_symbiote_offspring_pool -> damage -> add_card_to_hand`，确保致死剥离仍增长毒液并推进计数；子代死亡由各卡 `on_destroyed` 配置推进，不在死亡解析器里写卡名。
- 共生体能力边界。暴乱必须通过 `AttackAction.perform_attack(..., false)` 复用标准普通攻击结算，不能复制伤害流程；触发攻击不支付动作/攻击次数且禁止占领，但仍保留反伤、吸血、范围伤害、死亡归因和攻击后触发。沉默只由 `UnitSilenceResolver` 影响单位施法动作及被沉默英雄所属法术，不能顺带禁用主/副动作、被动或亡语。吞噬继承目标当前攻击、生命上限及固有关键词/动作/施法/触发，不复制运行时状态；屠杀通过标准 `on_kill` 事件、`CardState.runtime_counters` 和 `permanent_stat_overrides.attack_speed` 成长。修改后依次运行 `tools/test_symbiote_offspring_pool.gd`、`tools/test_symbiote.gd`、`tools/test_card_collection.gd`、卡牌校验和整项目检查。
