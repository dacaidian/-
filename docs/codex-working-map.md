...
- 东京喰种特效优先读 `scripts/ui/animation/tokyo_ghoul_animation_provider.gd`、`tokyo_ghoul_kagune_visual.gd`、`tokyo_ghoul_feather_visual.gd` 和 `scripts/ui/card_status_overlay.gd`。Provider 只处理路由、局部坐标、时长和回收；全战场画布负责雨夜都市、RC 脉冲与四赫子展开，羽针画布负责羽片投射物。持续覆盖必须读取状态 payload 的 `kagune_types` / `is_high_concentration` 快照：羽赫为尖锐羽片，甲赫为硬化甲壳，鳞赫为多条分节触肢，尾赫为单条稳定尾鞭；不要按卡牌 id 或当前 RC 反推，不要退回通用红色光圈、文字印章或相同直线触手。修改后运行 `tools/test_tokyo_ghoul_animation_provider.gd`、`tools/test_tokyo_ghoul.gd` 与 `tools/test_animation_routing.gd`。
