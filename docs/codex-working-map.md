...
- 暗夜精灵哨兵表现优先读 `scripts/ui/animation/night_elf_animation_provider.gd`、`scripts/ui/animation/night_elf_vfx_factory.gd`、`scripts/ui/animation/shaders/night_elf_*.gdshader`、`scripts/ui/card_status_overlay.gd` 和 `tools/test_night_elf_animation_provider.gd`。Provider 负责技能编排、贝塞尔路径、Tween 和根节点回收；Factory 负责 SDF 月体/新月/月束、星点、轨道、箭体、物理爪痕、双层轨迹与受控 CPU 粒子。不要再创建包办全部技能的单一 `_draw()` 画布。`moonblade` 必须是实体新月刃、双层尾迹、首次命中转刃和反向弹射；`claw_strike` 必须有暗色切口与银蓝锐边，不能是三条发光曲线；`tranquil_spring` 使用来源聚水、双层弧形水带、冷白月束、水纹与净化碎片；`meteor_strike` 的落点标记、下坠和冲击必须连续，群体共享根节点并轻量错峰。`precision_shot` / `meteor_aura` 持续层由 `CardStatusOverlay` 读取真实状态，避免全卡染色和常驻多层圆环。六种时间状态分别使用 `night_elf_time_transition_<state_id>` board key，规则层更新状态后发送当前 key，Provider 不读取或修改规则状态。新增技能先声明目标上下文和语义 key，再复用 Factory 图元，并运行 `tools/test_animation_routing.gd` 与 `tools/test_night_elf_animation_provider.gd`。
