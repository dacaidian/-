...
- 共生体能力边界。暴乱必须通过 `AttackAction.perform_attack(..., false)` 复用标准普通攻击结算，不能复制伤害流程；触发攻击不支付动作/攻击次数且禁止占领，但仍保留反伤、吸血、范围伤害、死亡归因和攻击后触发。沉默只由 `UnitSilenceResolver` 影响单位施法动作及被沉默英雄所属法术，不能顺带禁用主/副动作、被动或亡语。吞噬继承目标当前攻击、生命上限及固有关键词/动作/施法/触发，不复制运行时状态；屠杀通过标准 `on_kill` 事件、`CardState.runtime_counters` 和 `permanent_stat_overrides.attack_speed` 成长。`next_attack_modifier` 是通用下一击状态标签，伤害、固定恢复、建筑排除和表现 key 都来自 payload；`AttackAction` 不识别“撕咬”卡牌或状态 id。恐惧在 `before_turn_start` 前置状态阶段尝试八方向背离移动，不消耗行动、失败不改道，随后由 `action_prevention` 封锁整回合。
