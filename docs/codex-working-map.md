...
- 共生体。优先读 `data/cards.json` 中的 `symbiote.faction_card_pools` / `tokens[]`、`scripts/game/symbiote_offspring_pool_resolver.gd`、`scripts/game/unit_silence_resolver.gd`、`scripts/effects/update_symbiote_offspring_pool_effect.gd`、`random_normal_attacks_effect.gd`、`symbiote_whip_effect.gd`、`symbiote_absorb_effect.gd`、`carnage_kill_progress_effect.gd`、`scripts/actions/attack_action.gd`、`scripts/game/death_resolver.gd`、`scripts/data/card_state.gd` 和 `scripts/ui/animation/symbiote_animation_provider.gd`。静态配置只声明初始牌与解锁规则；玩家动态池仅保存在 `PlayerState.effect_runtime_values.symbiote_offspring_pool`，解锁必须通过稳定 rule id 幂等执行，不能把池进度写回 `CardData` 或 UI。剥离链顺序为 `play_animation -> gain_permanent_attack -> update_symbiote_offspring_pool -> damage -> add_card_to_hand`，确保致死剥离仍增长毒液并推进计数；子代死亡由各卡 `on_destroyed` 配置推进，不在死亡解析器里写卡名。当前只实现池和池内卡牌，不得假设已存在抽池、附着或部署流程。
