...
- 野兽人同系斩杀进化和卡扎克杀戮成长。优先读 `scripts/game/beastmen_evolution_resolver.gd`、`scripts/game/death_resolver.gd` 和 `data/cards.json` 中的 `evolution_rules` / `evolution_line`。进化规则放种族块，不要写死在单张随从里；规则展示牌可用 `start_in_hand` 默认入手。卡扎克成长由普通攻击击杀友方非英雄随从触发，不要做成新动作。野兽人卡牌可配置 `movement` 和 `chaos_corruption` 静态字段。
