...
- 共生体基础框架。当前只接入 `data/cards.json` 中的 `symbiote` 种族块和 `2/20` 英雄 `venom`，种族选择、英雄展示、图鉴和战场卡图都走现有数据驱动流程；标准卡图缺少 `-table` 版本时继续使用既有回退逻辑。`assets/img/共生体` 中其余图片只是后续卡牌美术储备，不代表规则已实现。自我剥离、子代池、宿主附着、子代进化和纳尔解放必须在需求明确后分别设计，不要从卡面文案推断并写死在英雄、UI 或 `GameManager` 中。
