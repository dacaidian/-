...
- 纳尔使用场上来源动态规则，不写进种族全局开关。囚禁/解放光环通过 `while_on_board + target_faction_id` 进入 `HandPassiveResolver` 快照；解放形态的组织扩展通过场上 `modify_hand_spell_effects` 把目标改为 `non_hero_minions`，并在替换效果上标记 `allow_any_non_hero_minion`。所有手牌可用性、目标高亮、AI 和执行路径都必须向 `HandSpellModifierResolver` 传 `GameManager`。法典的跨区域优先级固定为正面战场、手牌、坟场、隐藏棋盘、共享牌库，每次命中一个区域后立即返回，禁止复制唯一卡。修改后依次运行 `tools/test_symbiote_offspring_pool.gd`、`tools/test_symbiote.gd`、`tools/test_card_collection.gd`、卡牌校验、动画路由测试和整项目检查。
