...
- 在 `EffectRegistry` 注册。
- 效果层只做规则，不做 UI。
- 新的永久单位身份在棋盘生效时触发的效果使用 `on_enter_board`：包括翻开、手牌放置、死亡格召唤和不可逆永久进化。入口统一走 `GameManager.resolve_board_entry_triggers()` 并携带 `EventContext.BOARD_ENTRY_REASON`；真正翻牌额外兼容 `on_reveal`。保存原形快照的覆盖/限时变身和恢复原形不触发，禁止把可逆形态切换接入该入口刷取入场收益。
...
- 正面宽度攻击与数值关键字 `splash_N` 的固定溅射集中在 `AttackAction`。`frontal_width_N` 以攻击方向为中心命中面前奇数宽度 N 个格子的地面层和飞行层，`giant` 统一视为宽度 3；`ranged_attack_immune` 在同一入口过滤非贴身远程普通攻击，但不阻止近战、法术或效果。固定溅射扫描主目标周围八个相邻格，并同时读取每格地面层和飞行层；同格另一层不算相邻目标。新增类似普通攻击次级伤害时复用二次伤害和死亡归属链；表现通过 `GameAnimationResolver.play_secondary_attack_impact_animation()` 把攻击者、主目标及实际受伤目标快照交给 `CombatImpactAnimationProvider`，规则层不要重新扫描目标，也不要直接操作视觉节点。
...
- 手牌、装备与场上 `while_on_board` 持续被动统一由 `HandPassiveResolver.collect_active_passive_effects(player, game_manager)` 收集一次，再由各子刷新器消费；新增属性型被动时优先接入这个 snapshot，不要在多个刷新函数里重复扫描来源区域。
...
- `maintain_card_reserve.animation` 是一次补充批次的可选表现 key；只有实际获得至少一张牌时才通过全战场动画门面延迟播放一次，不得按补牌张数重复播放，也不得让同步库存状态机等待 UI。
...
- 狐妖仙尾数、献祭、魅惑、复生与狐火。优先读 `scripts/actions/sacrifice_faction_skill_action.gd`、`scripts/effects/sacrifice_friendly_minions_effect.gd`、`scripts/game/faction_skill_resolver.gd`、`scripts/ui/faction_skill_panel_controller.gd`、`scripts/ui/tail_resource_meter.gd`、`scripts/ui/card_status_overlay.gd`、`scripts/ui/animation/fox_spirit_animation_provider.gd` 及三个 `fox_spirit_*_visual.gd`。尾数只保存在 `PlayerState`，`TailResourceMeter` 只消费前后数值并绘制九槽、中央数字和 3/6/9 阈值；仅在数值变化期间运行24 FPS过渡，稳定后必须停止处理。种族 HUD 使用内容签名避免无变化重建。九尾生效后献祭仍使目标死亡，但收益改为给九尾作用范围授予复生；祸国必须逐个处理目标，并在每次献祭前重新读取修正，保证中途达到九尾后余下目标立即切换收益。规则层只请求稳定 animation key，不创建视觉节点。
...
- 东京喰种特效优先读 `scripts/ui/animation/tokyo_ghoul_animation_provider.gd`、`tokyo_ghoul_combat_visual.gd`、`tokyo_ghoul_ritual_visual.gd`、`tokyo_ghoul_event_visual.gd`、`tokyo_ghoul_kagune_visual.gd`、`tokyo_ghoul_feather_visual.gd` 和 `scripts/ui/card_status_overlay.gd`。Provider 是纯路由门面，只处理 key、局部坐标、时长和回收；Combat 负责普通攻击、吸血与反伤，Ritual 负责法术/动作/变身，Event 负责 RC 和随从库全战场反馈。`GameAnimationResolver.resolve_attack_animation_key()` 只根据攻击者当前静态卡牌数据选择表现轮廓，不参与伤害规则；远程赫子攻击替换通用弹道，近战赫子攻击保留通用撞击并叠加局部命中反馈。圣剑溅射与旧多/巨兽正面攻击的次级范围反馈走公共 `combat_impact_animation_provider.gd` / `combat_area_attack_visual.gd`，不能在东京喰种 provider 中复制范围规则。无目标法术若以 `owner_card_by_id` 唯一命中场上单位，并需要把单位作为动画落点，须在效果上显式配置 `presentation_target: "effect_target"`，再由 `HandPlayResolver.resolve_implicit_animation_target()` 解析；不要隐式改变其他无目标法术，也不要在效果执行后再补播一次同名特效。持续覆盖读取状态 payload 的 `kagune_types` / `is_high_concentration` 快照：羽赫为动态羽片，甲赫为咬合甲壳，鳞赫为摆动分节触肢，尾赫为稳定尾鞭；`lifesteal_animation` / `reflect_animation` 仅提供语义反馈，不能改变数值。覆盖形态用低干扰轮廓补充辨识，状态或形态移除后必须停止刷新。不要退回通用红色光圈、文字印章或相同直线触手。修改后运行 `tools/test_tokyo_ghoul_animation_provider.gd`、`tools/test_tokyo_ghoul.gd`、`tools/test_combat_impact_animation.gd` 与 `tools/test_animation_routing.gd`。
...
- 影月议会邪能基础体系。优先读 `scripts/game/spell_cast_trigger_resolver.gd`、`scripts/game/turn_event_ledger.gd`、`scripts/game/hand_spell_modifier_resolver.gd`、`scripts/game/hand_passive_resolver.gd`、`scripts/actions/spell_action.gd`、`scripts/actions/effect_action.gd`、`scripts/actions/attack_action.gd`、`scripts/game/granted_action_resolver.gd`、`scripts/game/hand_play_resolver.gd`、`scripts/game/spell_target_resolver.gd`、`scripts/game/status_resolver.gd`、`scripts/game/periodic_cycle_resolver.gd`、`scripts/effects/card_effect.gd`、`scripts/effects/life_drain_effect.gd`、`scripts/effects/destroy_units_effect.gd`、`scripts/effects/periodic_status_aura_effect.gd`、`scripts/effects/periodic_trigger_effect.gd`、`scripts/data/card_status.gd`、`scripts/data/card_state.gd`、`scripts/data/player_state.gd`、`scripts/ui/card_status_overlay.gd`、`scenes/card/scripts/card.gd`、`scripts/ui/card_animation_controller.gd` 和 `data/cards.json` 中的 `shadowmoon_council`。邪能法术或施法动作通过 `spell_tags: ["fel"]` 标记；默认入手升级牌“邪能狂乱”通过 `trigger: "after_spell_cast"` + `active_zone: "hand"` + `required_spell_tags` 监听成功施法，广播和六类随从响应统一配置 `once_per_turn_group: "shadowmoon_fel_madness"`。`SpellCastTriggerResolver` 必须先收集同一来源的整组匹配效果，再通过 `TurnEventLedger` 按玩家申领一次；首次邪能施法完整执行整组，当前回合后续邪能施法整组跳过，不能用状态去重代替这层事件闸门。生命吸取使用通用 `life_drain` 效果，按目标实际失去生命给指定 owner card 增加临时当前生命；该生命可超过上限，但不修改 `max_health`，也不触发有效治疗。需要让疯狂临时授予副动作时，把 `actions` 放在状态 `payload` 中，由 `GrantedActionResolver` 读取；需要临时授予关键字时，把 `keywords` 放在状态 `payload` 中，由 `CardState.has_keyword()` 读取，例如魅魔临时 `lifesteal`、末日守卫临时 `giant`；需要临时改写已有施法动作时，把 `spell_modifiers` 放在状态 `payload` 中，由 `HandSpellModifierResolver` 读取，例如术士疯狂后改写“诅咒”。装备改写施法动作时，使用 `modify_spell_ability` + `trigger: "while_equipped"`，由 `HandSpellModifierResolver` 从装备区读取；古尔丹之杖就是这个模式。地狱犬“法力燃烧”使用目标规则 `spellcaster_minions_or_heroes` 和表现 key `mana_burn`；术士“诅咒”使用目标规则 `non_hero_minions` 和通用状态 `damage_amplify`，该数值在 `CardState.take_damage()` 中并入同一次伤害事件，因此能被圣盾一次性格挡；地狱火“献祭”使用无目标施法动作 + `apply_status` + `target: "adjacent_enemy_non_hero_minions"` 施加 `fire`，火焰数值键为 `payload.fire_damage`，表现阶段为 `immolation_cast` / `immolation_mark` / `immolation_tick`；基尔加丹的低语使用 `periodic_status_aura`，黑暗之门使用 `periodic_trigger`，两者都用 `cycle_length` / `active_phases` / `runtime_state_id` 描述周期，并共用 `PeriodicCycleResolver` 把运行时相位保存在玩家 `effect_runtime_values`；翻出即触发且需要重建周期的来源使用 `reset_phase: true`；混乱狼骑兵“撕咬”使用目标规则 `adjacent_minions`、表现 key `fel_bite`，并通过同一个 `EffectAction` 同时执行伤害和自我治疗；古尔丹之杖把“邪能灌注”升级为 `fel_overload`，通过状态 `turn_effects` 和通用 `destroy_units` 实现回合结束爆裂。“恶魔召唤”和“黑暗之门”都使用 `add_card_to_hand` 获取同族恶魔/邪兽人牌，其中魅魔来自同族 `tokens[]`。嘲讽关键字 `taunt`、巨兽关键字 `giant` 和吸血关键字 `lifesteal` 属于普通攻击规则，入口都在 `AttackAction`，不要在 UI 或单张卡牌里写死。新增更多疯狂分支、周期光环或周期事件时优先增加通用配置，不要在 `SpellAction`、`GameManager` 或具体随从代码中按卡牌 id 写死。
- 影月议会表现层优先读 `scripts/ui/animation/shadowmoon_animation_provider.gd`、`shadowmoon_target_visual.gd`、`shadowmoon_ritual_visual.gd`、`shadowmoon_vfx_palette.gd`、`scripts/effects/play_animation_effect.gd`、`scripts/effects/apply_status_effect.gd`、`scripts/effects/damage_effect.gd`、`scripts/effects/periodic_status_aura_effect.gd` 和 `scripts/ui/card_status_overlay.gd`。需要“代价 -> 传输 -> 状态 -> 广播”顺序时，在卡牌效果数组中插入纯表现 `play_animation`，并用 `presentation_scope` 选择 `source_to_target`、`multi` 或 `board`；不要让 Provider 猜测结算顺序。群体疯狂、周期低语、地狱火附着和爆裂命中必须通过一次 `multi_rect` 编排，缺少路由才回退逐目标。疯狂广播和六类单位响应使用不同 key，持续覆盖读取真实状态。修改后运行 `tools/test_shadowmoon_animation_provider.gd`、动画路由测试、卡牌校验和整项目检查。
- 影月议会材质主体必须是黑紫膜包裹的黏稠压力核心，传输会鼓胀与收束，恶魔裂隙有内陷空间膜，疯狂广播是掠过棋盘的低矮压力前沿；灵魂使用蓝紫流体，诅咒与低语使用暗红紫。酸绿直线、空心圆和全卡绿色闪烁只允许作次级读数，不能重新承担法术主体。灌注、过载、疯狂、伤害加深和低语由 `CardStatusOverlay` 一次绘制后使用合成透明度呼吸，禁止继续 `_process()` 重建几何；地狱火保留12 FPS形态动画。复杂一次性 Visual 继承 `ThrottledProgressVisual` 并按24 FPS合并重绘。共享 Ribbon 过滤非有限、重复和子像素退化路径，并用面积校验后的直接四边形；共享柔光椭圆使用单位圆绘制变换并过滤不可见半径，禁止恢复 `draw_colored_polygon()` 三角化路径。窄线和复制量大的轮廓使用 `draw_stroked_path`。调整后运行两项共享 VFX 测试和影月专项测试。
- 共生体。优先读 `data/cards.json` 中的 `symbiote.faction_card_pools` / `tokens[]`、`scripts/game/symbiote_offspring_pool_resolver.gd`、`scripts/game/unit_silence_resolver.gd`、`scripts/game/hand_passive_resolver.gd`、`scripts/game/hand_spell_modifier_resolver.gd`、`scripts/game/status_resolver.gd`、`scripts/effects/update_symbiote_offspring_pool_effect.gd`、`attach_symbiote_offspring_effect.gd`、`terrifying_scream_effect.gd`、`liberate_card_effect.gd`、`random_normal_attacks_effect.gd`、`symbiote_whip_effect.gd`、`symbiote_absorb_effect.gd`、`carnage_kill_progress_effect.gd`、`scripts/actions/attack_action.gd`、`scripts/game/death_resolver.gd`、`scripts/data/card_data.gd`、`scripts/data/card_state.gd`、`scripts/ui/animation/symbiote_animation_provider.gd`、`symbiote_severance_visual.gd`、`symbiote_power_visual.gd`、`symbiote_combat_visual.gd` 和 `symbiote_event_visual.gd`。静态配置声明初始牌、解锁规则和 `with_replacement_card_ids`；玩家动态池仅保存在 `PlayerState.effect_runtime_values.symbiote_offspring_pool`，初始牌只初始化一次，后续查询绝不能补回不放回卡。解锁必须通过稳定 rule id 幂等执行，不能把池进度写回 `CardData` 或 UI。共生体组织默认只选择 `target_card_ids` 中的己方宿主；目标二次校验后才抽池，共生体战士保留，其余结果移除，然后使用无 transform 状态的 `transform_to_card_data()` 永久替换宿主并保留已消耗行动。`inherit_host_base_stats_card_ids` 声明基因战士等特殊宿主：替换前读取卡牌基础攻击/生命，替换后叠加并写入永久属性覆盖，再刷新新旧控制者的持续被动，使实时光环最后应用，不得用状态模拟。动画不得提前抽池。剥离链顺序为 `play_animation -> gain_permanent_attack -> update_symbiote_offspring_pool -> damage -> add_card_to_hand`，确保致死剥离仍增长毒液并推进计数；子代死亡由各卡 `on_destroyed` 配置推进，不在死亡解析器里写卡名。
- 共生体能力边界。暴乱必须通过 `AttackAction.perform_attack(..., false)` 复用标准普通攻击结算，不能复制伤害流程；触发攻击不支付动作/攻击次数且禁止占领，但仍保留反伤、吸血、范围伤害、死亡归因和攻击后触发。沉默只由 `UnitSilenceResolver` 影响单位施法动作及被沉默英雄所属法术，不能顺带禁用主/副动作、被动或亡语。吞噬继承目标当前攻击、生命上限及固有关键词/动作/施法/触发，不复制运行时状态；屠杀通过标准 `on_kill` 事件、`CardState.runtime_counters` 和 `permanent_stat_overrides.attack_speed` 成长。眠者“获得猫”与沉默显现使用 `on_enter_board`，共生体组织完成永久替换和被动刷新后再发布 `BOARD_ENTRY_PERMANENT_TRANSFORM`，不能为眠者写卡牌 id 特判，也不能让可逆变身触发。`next_attack_modifier` 是通用下一击状态标签，伤害、固定恢复、建筑排除和表现 key 都来自 payload；`AttackAction` 不识别“撕咬”卡牌或状态 id。恐惧在 `before_turn_start` 前置状态阶段尝试八方向背离移动，不消耗行动、失败不改道，随后由 `action_prevention` 封锁整回合。
- 纳尔使用场上来源动态规则，不写进种族全局开关。囚禁/解放光环通过 `while_on_board + target_unit_traits: ["symbiote"]` 进入 `HandPassiveResolver` 快照；`faction_id: "symbiote"` 仅表示卡包归属，人类宿主、异噬体和纳尔都不声明该 `unit_traits`，因此不会获得光环。解放形态的组织扩展通过场上 `modify_hand_spell_effects` 把目标改为 `non_hero_minions`，并在替换效果上标记 `allow_any_non_hero_minion`。所有手牌可用性、目标高亮、AI 和执行路径都必须向 `HandSpellModifierResolver` 传 `GameManager`。法典的跨区域优先级固定为正面战场、手牌、坟场、隐藏棋盘、共享牌库，每次命中一个区域后立即返回，禁止复制唯一卡。修改后依次运行 `tools/test_symbiote_offspring_pool.gd`、`tools/test_symbiote.gd`、`tools/test_card_collection.gd`、卡牌校验、动画路由测试和整项目检查。
- 共生体表现遵循“规则确认变化后发布语义 key，Provider 只负责路由与生命周期”。Severance 负责剥离/附着，Power 负责毒液英雄法术，Combat 负责普通攻击和暴乱/嚎叫/皮鞭/吞噬，Event 负责池解锁、子代出壳、屠杀成长、沉默、眠者与纳尔光环。共生体近战必须保留通用卡牌撞击，再叠加局部生物兵器命中；`GameAnimationResolver` 只给实际声明 `unit_traits: ["symbiote"]` 的单位选择种族攻击 key。规则事件不得仅因效果被调用就播放：池解锁、屠杀阈值、吞噬成长与附着完成必须先验证真实状态变化。`play_animation.target_unit_traits` 只允许过滤表现目标，不能代替纳尔的被动规则。吞噬继承和沉默光环的常驻覆盖采用一次绘制加合成透明度呼吸，禁止逐帧重建。修改后依次运行共生体两项测试、动画路由、两项共享 VFX 测试、卡牌校验和整项目检查。
...
- 程序化主题材质优先复用 `scripts/ui/animation/vfx_canvas_toolkit.gd`：柔光体、雾膜、光点和有机丝带只负责绘制质感，不注册 key、不创建 Tween、不读取规则。急弯与闭环必须使用其分段安全丝带，禁止恢复整条自交多边形；也不要重新堆三层等宽 `draw_line` / `draw_arc` 充当技能主体。颜色、形状、节奏和语义仍归各族 Visual 所有，不能把共享工具变成统一模板特效。
...
- 手牌四区布局优先读 `scenes/ui/hand_drawer_panel.tscn`、`scripts/ui/hand_drawer_controller.gd` 和 `scripts/ui/hand_section_layout_policy.gd`。分区高度只由净可用高度、卡牌数量和每行容量决定：空区折叠，非空区按内容需求加权分配；每区只允许纵向滚动，横向滚动必须禁用。横纵预算都从视口约束后的目标抽屉尺寸扣除皮肤安全区、内部留白及对应容器间距得到，不能在抽屉刚缩放后读取上一帧的 `Sections.size`。不要重新给四个 section 设置 `EXPAND_FILL`，也不要让焦点卡牌或动作菜单参与高度权重。视口缩放时先捕获纵向偏移并停止 Tween，再按 `is_open` 重算抽屉位置、刷新所有既有 `CardFlow` 宽度、分配高度并恢复偏移。修改算法后运行 `tools/test_hand_section_layout.gd` 和 `tools/test_hand_drawer_layout.gd`；后者必须覆盖窄视口四区同时非空、收起状态缩放、卡牌换列和滚动位置保持。
...
- 猴妖仙表现优先读 `scripts/ui/animation/monkey_animation_provider.gd`、`monkey_spell_visual.gd`、`scripts/ui/card_status_overlay.gd`、`scripts/game/game_animation_resolver.gd` 和移动/攻击动作。Provider 只负责路由、坐标、时序和回收；`MonkeySpellVisual` 负责赤金笔势、卷云、火眼扫描、猴毛剪影、铜甲、蟠桃、驱神、定字封印、龙宫神器与法象巨影。笔势主体使用非均匀宽度与飞白高光，筋斗云使用有内流明暗的玉白云体，火眼扫描使用透光幕而非几条竖线。筋斗云/西行是在通用位移动画上叠加路径；毫毛协攻保留通用近战撞击，专属 key 只绘制目标附近的棍痕与分身命中，不绘制跨场远程轨迹；铜头铁臂反弹使用稳定事件 key。铜头铁臂、蟠桃、定身、聚散成气、火眼回合标记与筋斗云持续反馈属于 `CardStatusOverlay`；法象仅在变身瞬间显示巨影，变身后依靠替换卡面和巨兽标识，不添加持续遮罩。新增技能应扩展语义 key 和绘制函数，不得退回 Panel、单字 Label、通用光圈或等宽线框；修改后运行 `tools/test_monkey_animation_provider.gd` 与 `tools/test_animation_routing.gd`。
...
- 野兽人表现优先读 `scripts/ui/animation/beastmen_animation_provider.gd`、`beastmen_combat_visual.gd`、`beastmen_ritual_visual.gd`、`beastmen_path_visual.gd`、`beastmen_vfx_palette.gd` 与 `scripts/ui/card_status_overlay.gd`。Provider 是稳定路由门面，只处理表现上下文、坐标、时序和回收；Combat 负责同系进化、卡扎克杀戮成长、万魔充能命中和咆哮受益；Ritual 负责野蛮咆哮、野性呼唤、兽径起手、废灭仪式和全战场混沌腐蚀；Path 只负责多格连续挖掘；Palette 固定深土、干血、兽骨、铜锈、暗橙及少量混沌紫绿。持续兽径、混沌腐蚀数值、万魔充能和咆哮战纹由 `CardStatusOverlay` 读取实时数据贴边绘制，不得遮挡卡名与战斗数值。
- `savage_roar` 只播放一次来源战吼，群体状态施加必须使用 `savage_roar_buff` 做逐目标短反馈，禁止每个友军重复播放全场咆哮。`beastmen_evolution` / `beastmen_slaughter` 在死亡链完成后表现骨角生长与血脉吸收，不替代通用近战撞击和真实死亡；`chaos_corruption_burst` 必须走 board 路由，`beast_path` 的挖掘必须走 path 路由。兽径权威数据仍在 `BoardCell`，卡面覆盖只消费 `CardState.has_beast_path` 显示镜像。新增或调整野兽人表现后运行 `tools/test_beastmen_animation_provider.gd`、`tools/test_animation_routing.gd` 与卡牌校验。
- 狐妖仙表现优先读 `scripts/ui/animation/fox_spirit_animation_provider.gd`、`fox_spirit_target_visual.gd`、`fox_spirit_area_visual.gd`、`fox_spirit_ritual_visual.gd`、`scripts/ui/animation/throttled_progress_visual.gd`、`scripts/ui/tail_resource_meter.gd` 和 `scripts/ui/card_status_overlay.gd`。Provider 只做路由、坐标、锚点、时序和回收；Target 负责献祭/复生授予/勾魄/两类控制/衍生牌入场，Area 只负责精确 2x2 狐火，Ritual 负责祸国/九尾军势/天狐化。三种 Visual 统一按24 FPS重建复杂几何；狐尾、魂丝和控制权丝带使用有厚度的分层描边与柔光，目标周围的香雾膜与错相光点只建立纵深，不遮挡卡面。尾数仪表静态阶段停止处理；永久魅惑、狐念之术和勾魄通过一次绘制加合成透明度呼吸保持动态，禁止使用爱心表达。祸国的 `ruin_country_targets` 必须消费 `SacrificeFriendlyMinionsEffect` 已解析的真实目标矩形，只标记友方非英雄随从，不得用固定 5x5 图案伪造英雄、敌方或空格目标。`EvolveUnitsEffect.animation`、`GrantRebornEffect.animation`、`AddCardToHandEffect.animation` 与随从 `CardData.animation` 是通用表现钩子，不得在效果类或 `HandPlayResolver` 中按狐妖卡牌 id 分支。九尾用 `fox_reborn` 表现授予，其他来源的 `reborn` 不得被狐妖 provider 接管。普通献祭先指向可见尾数仪表，再由同一动画根节点回勾到场上苏妲己；九尾献祭使用独立月白复生反馈，不得伪造资源继续增长。修改后运行 `tools/test_fox_spirit_animation_provider.gd`、两项共享 VFX 测试、`tools/test_animation_routing.gd` 与 `tools/test_right_side_hud.gd`。
...
- 暗夜精灵哨兵表现优先读 `scripts/ui/animation/night_elf_animation_provider.gd`、`night_elf_kinetic_vfx.gd`、`night_elf_support_vfx.gd`、`night_elf_celestial_vfx.gd`、`night_elf_time_vfx.gd`、`night_elf_vfx_runtime.gd`、`night_elf_vfx_factory.gd`、`scripts/ui/animation/shaders/night_elf_*.gdshader`、`scripts/ui/card_status_overlay.gd` 和两项暗夜精灵动画测试。Provider 只做上下文校验和语义分派；Kinetic 负责月刃/爪击，Support 负责治疗/强化，Celestial 负责满月/流星，Time 负责时间切换；Runtime 负责时长、坐标、贝塞尔和根节点回收；Factory 只创建图元。私有语义模块不得添加全局 `class_name`，也不得读取规则状态。`moonblade` 必须是实体新月刃、双层尾迹、首次命中转刃和反向弹射；`claw_strike` 必须有暗色切口与银蓝锐边，不能是三条发光曲线；`tranquil_spring` 使用来源聚水、双层弧形水带、冷白月束、水纹与净化碎片；`meteor_strike` 的落点标记、下坠和冲击必须连续，群体共享根节点并轻量错峰。`precision_shot` / `meteor_aura` 持续层由 `CardStatusOverlay` 读取真实状态。六种时间状态分别使用 `night_elf_time_transition_<state_id>` board key，规则层更新状态后发送当前 key。修改后运行 `tools/test_night_elf_vfx_modules.gd`、`tools/test_animation_routing.gd` 与 `tools/test_night_elf_animation_provider.gd`。
- 新增或迁移动画 key 后运行 `python tools/validate_cards.py` 和 `tools/test_animation_routing.gd`；前者扫描中央控制器、provider 的 `*_KEYS` 数组和 `*_ANIMATION_KEY` 常量，后者验证 provider 的上下文路由契约。拥有复杂自绘或持续刷新生命周期的主题还应提供独立测试，至少验证动画节点完整释放、状态移除后停止处理；达拉然使用 `tools/test_dalaran_animation_provider.gd`，苗疆使用 `tools/test_miao_animation_provider.gd`，狐妖仙使用 `tools/test_fox_spirit_animation_provider.gd`。
- UI 控制器不拥有玩法规则。
