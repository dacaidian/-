...
- 共生体。优先读 `data/cards.json` 中的 `symbiote`、`scripts/actions/spell_action.gd`、`scripts/effects/play_animation_effect.gd`、`scripts/effects/damage_effect.gd`、`scripts/effects/add_card_to_hand_effect.gd`、`scripts/ui/animation/symbiote_animation_provider.gd` 和 `symbiote_severance_visual.gd`。当前牌池包含毒液、共生神盾局探员、生物研究员、异噬体和基因战士，`symbiote_tissue` 是 `tokens[]` 中不可打出的无效果衍生法术。自我剥离和人工剥离使用 `effect_handles_animation: true`，并按 `play_animation -> damage -> add_card_to_hand` 顺序执行；人工剥离用 `owner_card_by_id: venom` 的表现步骤同时承担毒液在场校验，不要改成无条件生成组织。`SpellAction` 会在每个运行时子效果上固化 `effect_owner_id`，保证来源中途死亡后后续玩家级效果仍有归属。表现 Provider 只处理两个语义键；自我剥离走目标矩形，人工剥离走来源到毒液的定向路由，规则和动画不得互相查询。后续子代池、宿主附着、进化和纳尔解放必须另行设计。修改后运行 `tools/test_symbiote.gd`、`tools/test_card_collection.gd`、卡牌校验和整项目检查。
