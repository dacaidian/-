...
- 猴妖仙表现优先读 `scripts/ui/animation/monkey_animation_provider.gd`、`monkey_spell_visual.gd`、`scripts/ui/card_status_overlay.gd`、`scripts/game/game_animation_resolver.gd` 和移动/攻击动作。Provider 只负责路由、坐标、时序和回收；`MonkeySpellVisual` 负责赤金笔势、卷云、火眼扫描、猴毛剪影、铜甲、蟠桃、驱神、定字封印、龙宫神器与法象巨影。筋斗云/西行是在通用位移动画上叠加路径；毫毛协攻保留通用近战撞击，专属 key 只绘制目标附近的棍痕与分身命中，不绘制跨场远程轨迹；铜头铁臂反弹使用稳定事件 key。铜头铁臂、蟠桃、定身、聚散成气、火眼回合标记与筋斗云持续反馈属于 `CardStatusOverlay`；法象仅在变身瞬间显示巨影，变身后依靠替换卡面和巨兽标识，不添加持续遮罩。新增技能应扩展语义 key 和绘制函数，不得退回 Panel、单字 Label 或通用光圈；修改后运行 `tools/test_monkey_animation_provider.gd` 与 `tools/test_animation_routing.gd`。
