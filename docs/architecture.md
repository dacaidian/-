...
- SSS 阶随从“芳村功善”为 4/12：主动作“免单”使用通用 `friendly_minions_by_faction` 目标集合恢复全部己方东京喰种，并通过 `set_faction_runtime_state` 把 RC 浓度置为高；副动作“赫者化”每个单位实例仅可使用一次，永久覆盖变身为 4/10、巨兽、攻城 4 的“不杀之枭”。“高槻泉”为 5/10、瞬移、移动攻击，也可一次性覆盖变身为 12/12 巨兽“独眼之枭”。两种枭形态被击败时均按覆盖变身规则恢复各自原形，且一次性动作消耗不会因恢复原形而重置。“旧多二福”为 8/8，使用 `frontal_width_5` 同时攻击面前五格，并具有 `magic_immune` 与 `ranged_attack_immune`。“死堪”为 6/14，同时拥有尾赫、鳞赫、甲赫与羽赫；`KagunePowerResolver` 会收集并合并多个赫子 payload，因此赫子解放期间四种能力同时生效，不需要死堪专用分支。`TokyoGhoulAnimationProvider` 负责免单与赫者化演出。
