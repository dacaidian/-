...
东京喰种表现层采用“规则快照 -> 路由编排 -> 专用自绘画布/持续覆盖”边界。`KagunePowerResolver` 在 payload 中同时固化 `kagune_types` 与 `is_high_concentration`，表现层不得根据卡牌 id、当前 RC 或数值加成反推本次释放形态。`TokyoGhoulAnimationProvider` 只注册 animation key、换算局部坐标并管理 Tween/临时节点；`TokyoGhoulKaguneVisual` 负责全战场“压抑蓄势 -> RC 脉冲 -> 赫子破出 -> 主体展开 -> 残留余波”，`TokyoGhoulFeatherVisual` 负责羽针的羽片喷射和穿刺命中。持续状态仍由 `CardStatusOverlay` 消费真实状态快照，以羽片、层叠甲壳、分节触肢和带骨刺尾鞭分别表示羽赫、甲赫、鳞赫和尾赫；高浓度只增强呼吸频率、材质亮度与边缘强度，不改变规则。统一色彩是深血红、酒红、紫黑与冷白边缘，都市蓝紫反光和雨线只作环境辅助，禁止退回通用红色光圈、文字印章、魔法阵或无类型差异的直线触手。修改这组表现后必须运行 `tools/test_tokyo_ghoul_animation_provider.gd`、`tools/test_tokyo_ghoul.gd` 和路由测试。
...
手牌抽屉仍按法术、随从、升级、装备四个语义分区，并保留各自独立的纵向滚动，但不再固定四等分。所有分区禁止横向滚动；卡牌流宽度必须从 `Sections` 当前真实宽度扣除 `SECTION` 安全区和内容间距后计算，不得复用抽屉旧固定宽度。生产节点树位于独立场景 `scenes/ui/hand_drawer_panel.tscn`，主场景和 UI 集成测试共用同一组件。`HandSectionLayoutPolicy` 是不依赖节点的纯布局策略：空分区收缩为仅标题的窄条；非空分区先取得一致的可操作最低高度，再按实际卡牌行数的平方根分配剩余空间，使拥挤区明确获得更多高度，同时避免卡牌数量极端悬殊时独占抽屉；某分区达到完整内容高度后停止增长，空间继续分给仍需滚动的分区。`HandDrawerController` 只统计卡牌数、计算每行容量并应用高度；焦点变化不参与高度计算，因此点击卡牌不会引发布局跳动。完整重建前捕获四区纵向滚动偏移；旧滚动节点会先脱离父节点再延迟释放，避免与新节点同名；随后在新高度应用并完成容器重排后统一恢复偏移。窗口尺寸变化只重新运行布局策略，不重新构造卡牌节点。

抽屉外框的宽高必须先约束到当前可见视口。四区的纵向分配预算由目标抽屉高度依次扣除 `DRAWER` 皮肤安全区、内部 `MarginContainer` 留白、标题高度及 `VBoxContainer` 间距得到；禁止在刚修改抽屉尺寸后直接读取 `Sections.size.y` 作为预算，因为 Godot 容器重排前该值仍可能属于上一帧，把它回写到分区最小高度会形成尺寸反馈并把末尾分区推到屏幕外。抽屉、主体和分区容器启用内容裁剪作为重排期间的显示保护，但裁剪不能替代正确的预算计算。窄视口回归测试必须同时填满四个分区，并验证抽屉边界、分区总高度及末尾装备区都处于视口内。
