...
`RightSideHudStyle` 提供右侧面板共享的背景、边框、标题、按钮、指标块和资源刻度样式；`HudSymbolIcon` 提供不依赖种族素材的轻量矢量图标。面板标题和“法力、翻牌、资源分、装备类型、种族状态、种族技能”等稳定语义优先使用图标与 tooltip，精确玩法数值仍保留短格式。通用的小上限资源优先用离散刻度，较大资源使用进度条；如果资源本身拥有不可替代的形状语义，可以提供只读专属仪表，但必须继续嵌入统一 HUD 外壳并由 controller 传入前后数值。`TailResourceMeter` 是首个例子：九个尾槽、中央数字和 3/6/9 阈值属于表现，尾数增减仍只由玩家状态决定。各 controller 可以选择自己的强调色，但不得复制一套新的面板外壳、宽度或定位常量。
...
程序化 2D 特效的共享材质原语位于 `scripts/ui/animation/vfx_canvas_toolkit.gd`。它只在主题 Visual 或 `CardStatusOverlay._draw()` 内提供柔光体、椭圆雾膜、分层光点、带宽度变化的有机丝带和弧形丝带，不拥有 animation key、Tween、节点生命周期或任何规则状态。丝带按安全四边形分段绘制并用柔性接缝补齐，不能退回会在急弯/闭环自交的整条多边形，也不要用三层等宽 `draw_line` / `draw_arc` 重新模拟材质。主题层负责选择颜色、形状、节奏和语义：共享工具只解决厚度、柔边、呼吸与高光，不应把不同种族统一成同一种光效。
...
持续表现继续读取真实数据。`TailResourceMeter` 只缓存上一帧和当前值用于过渡，不拥有资源；1-2、3-5、6-8、9 尾分别使用暗红、胭脂紫、月白紫缘和珍珠金阶段色，并同时显示中央数字与 3/6/9 阈值。九条尾槽使用低频错相呼吸，静止数值也保持极轻的材质流动，但不改变资源、阈值或布局。普通献祭在魂魄进入仪表后，再沿同一临时根节点向场上苏妲己回勾一缕妖气；Provider 返回后规则层才更新尾数、被动与攻击数字，从而形成连续反馈而不让 UI 提前修改数值。勾魄状态由 `CardStatusOverlay` 在攻击力区域绘制魂影、丝线、裂纹和持续刻度；永久魅惑使用完整控制权边框与睁开的狐眼，狐念之术使用保留原阵营底色的双色临时边框、不完全狐眼和倒计时。状态移除后覆盖必须立即停止，施法动画不能承担控制权生命周期。
