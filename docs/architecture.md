...
`RightSideHudLayoutController` 只负责排列已经创建好的右侧 HUD 面板，例如回合状态、种族技能、种族时间和装备展示。列宽根据视口在 296px 至 340px 之间变化；布局先保留各面板自然高度，高度不足时压缩间距但禁止互相覆盖，三块及以上面板且空间充足时按权重共享剩余高度，一至两块面板只获得有限留白，避免形成巨大空框。余量以整数像素分配，最后一个面板吸收余数，防止 720p 下累计取整越界。它不决定面板是否可见，也不读取或修改玩法状态；`GameManager` 只把需要参与布局的 panel 列表交给它。
