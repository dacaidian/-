...
- 共生体：已接入 `symbiote` 种族块和 `2/20` 的 1 阶英雄 `venom`（毒液），种族选择与图鉴直接消费这份卡牌数据。当前图片目录包含组织、子代、人类宿主、纳尔和相关法术的美术储备，但尚未定义玩法数据。卡面中的自我剥离、撕咬、恐怖尖啸、法典与子代判定目前不是已实现能力。后续实现应把“组织剥离”“有限子代池”“宿主附着/脱离”“子代进化”和“纳尔解放”拆成独立规则边界，避免在毒液英雄卡或 UI 中按具体子代写死。
...
- 共生体：基础种族与毒液英雄已经接入；组织剥离、子代池进化、附着到人类随从后入场以及纳尔的后期全局强化仍属于后续机制设计。
