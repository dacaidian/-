...
持续表现继续读取真实数据。`TailResourceMeter` 只缓存上一帧和当前值用于过渡，不拥有资源；1-2、3-5、6-8、9 尾分别使用暗红、胭脂紫、月白紫缘和珍珠金阶段色，并同时显示中央数字与 3/6/9 阈值。普通献祭在魂魄进入仪表后，再沿同一临时根节点向场上苏妲己回勾一缕妖气；Provider 返回后规则层才更新尾数、被动与攻击数字，从而形成连续反馈而不让 UI 提前修改数值。勾魄状态由 `CardStatusOverlay` 在攻击力区域绘制魂影、丝线、裂纹和持续刻度；永久魅惑使用完整控制权边框与睁开的狐眼，狐念之术使用保留原阵营底色的双色临时边框、不完全狐眼和倒计时。状态移除后覆盖必须立即停止，施法动画不能承担控制权生命周期。
