...
- 覆盖变身：持续结束恢复原形；持续期间死亡时先解除变身并恢复原形，不进入真正死亡。变身可用 `preserve_original_identity` 控制是否继续代表原卡牌，默认 `true` 以兼容已有英雄变身；设为 `false` 时，`represents_card_id()`、英雄在场检查和按原卡牌 id 授予的能力都只识别当前形态。东京喰种“蜈蚣形态”和“龙形态”均为永久覆盖变身，分别生成 3/6、攻速 2、移动力 3、移动攻击的蜈蚣形态，以及 5/8、巨兽的龙形态；二者不再代表金木研，因此不能继续施放金木研所属法术，也不能在形态之间嵌套覆盖；被击败时恢复变身前的金木研快照。
