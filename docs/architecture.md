...
统一界面皮肤分为三层。`GameUiSkin` 是纯资源层，只预载 `assets/img/ui_skin/` 中的位图，创建带固定纹理边距和安全内容区的 `StyleBoxTexture`，并提供主面板、内嵌面板、紧凑 HUD、输入框以及按钮普通/悬浮/按下/禁用状态；它不识别具体页面、玩法状态或按钮文案。`PANEL_TEXTURE_MARGINS` 只保护九宫格不被拉坏，`PANEL_SAFE_INSETS` 才定义内容不得侵入的木框、金属角和内斜面范围，两者不能互相替代；调用方传入的 padding 只能追加，不能清空基础安全区。`ApplicationUiStyle` 是应用页面语义层，负责主菜单、种族选择、图鉴、功能占位页和结算页的主次按钮、危险按钮、面板与输入框。`RightSideHudStyle` 是战斗 HUD 语义层，负责紧凑尺寸、强调色、指标块和资源刻度，外层框体仍复用统一位图。各页面 controller 只能请求这些稳定变体，不得各自复制大型 `StyleBoxFlat` 木框、金属框或按钮状态。

皮肤母版通过 `tools/build_ui_skin_assets.py` 构建为运行时资源。生成母版可以保留宽阔装饰边缘，但用于筛选区、HUD、按钮和输入框的运行时纹理必须在构建阶段压成较薄高度，不能把大型概念框原样塞进 32–180px 控件。横向按钮的四态必须来自同一轮廓，避免悬浮时布局跳动；按下态只通过图像反馈和内容轻微下移表达；手牌抽屉侧边按钮使用独立纵向 tab 资源，不能旋转横向按钮或强行拉伸文字。`PanelContainer` 由 StyleBox 自动落实安全区；普通 `Panel` 不会据此布局子节点，因此手牌抽屉等普通 Panel 必须在 controller 中显式维护与框体匹配的内容矩形。主面板内部可以嵌套轻量内嵌槽，但禁止在装饰性面板内继续堆叠同等级装饰框；HUD 的数值 chip 保留克制的代码样式，以保证信息密度。新增或重建 PNG 后，先运行 `tools/run_godot_validation.ps1 -ImportAssets`，再运行 `tools/test_ui_skin.gd` 验证纹理尺寸、状态差异、安全内容区、场景接入和九宫格契约。
