...
东京喰种表现层采用“规则快照 -> 语义键 -> 路由门面 -> 专用自绘画布/持续覆盖”边界。`KagunePowerResolver` 在 payload 中固化 `kagune_types`、`is_high_concentration` 以及吸血/反伤的表现 key，规则数值不依赖动画实现。`TokyoGhoulAnimationProvider` 只注册路由、换算坐标、统一时长与回收临时节点；`TokyoGhoulCombatVisual` 负责四赫子、复合赫子、蜈蚣、龙、圣剑、枭和旧多正面五格的普通攻击，并由 `GameAnimationResolver.resolve_attack_animation_key()` 根据当前卡牌数据选择轮廓。东京喰种专属攻击是通用近战/远程动画的替代项，不能先播放一次通用弹道再叠加主题动画。`TokyoGhoulRitualVisual` 负责变身、赫者化、恢复原形、尾赫齐发、分食、免单、特调和方糖咖啡；`TokyoGhoulEventVisual` 负责 RC 升降与 S/SSS 随从库补充；`TokyoGhoulKaguneVisual` 与 `TokyoGhoulFeatherVisual` 分别负责全战场赫子解放和羽针。无目标法术若通过 `target: "owner_card_by_id"` 明确作用于唯一场上单位，并显式配置 `presentation_target: "effect_target"`，`HandPlayResolver.resolve_implicit_animation_target()` 才把该规则目标解析为表现落点，使手牌到单位播放一次完整施法；未声明的既有法术仍保留原来的手牌位置演出。该元数据不改变选中、合法性或效果目标。持续赫子覆盖必须消费真实状态快照，以羽片、层叠甲壳、动态分节触肢和摆动尾鞭区分四类赫子；高浓度只增强密度与节奏。覆盖形态另有低干扰动态轮廓，状态或形态移除后立即停止刷新。统一色彩是深血红、酒红、紫黑与冷白边缘，都市蓝紫反光和雨线只作辅助，禁止退回通用红色光圈、文字印章、魔法阵或无类型差异的直线触手。修改后必须运行 `tools/test_tokyo_ghoul_animation_provider.gd`、`tools/test_tokyo_ghoul.gd`、`tools/test_rc_concentration.gd`、`tools/test_card_reserve.gd` 和路由测试。
