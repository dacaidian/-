...
- 覆盖变身：持续结束恢复原形；持续期间死亡时先解除变身并恢复原形，不进入真正死亡。变身可用 `preserve_original_identity` 控制是否继续代表原卡牌，默认 `true` 以兼容已有英雄变身；设为 `false` 时，`represents_card_id()`、英雄在场检查和按原卡牌 id 授予的能力都只识别当前形态。东京喰种“蜈蚣形态”“龙形态”和“圣剑形态”均为永久覆盖变身，分别生成 3/8、攻速 2、移动力 3、移动攻击的蜈蚣形态，6/8、巨兽的龙形态，以及 8/14、远程、溅射 4 的圣剑形态；三者不再代表金木研，因此不能继续施放金木研所属法术，也不能在形态之间嵌套覆盖；被击败时恢复变身前的金木研快照。覆盖形态也可以通过通用 `restore_transform` 效果主动结束；该效果只接受有效覆盖变身，并统一调用 `CardState.restore_from_transform_status()`，与到期和死亡恢复共享原形快照及行动经济规则。
...
- 东京喰种：`runtime_state.transition_policy: "rc_concentration"` 使用 `RcConcentrationResolver` 消费 `TurnEventLedger` 的标准化死亡记录。初始为高浓度；当前行动玩家每击杀一个敌方非英雄随从便立即提升一级，同一回合的多次独立击杀可从低连续升至高；高浓度不再继续增长。回合结束时若整回合没有合格杀戮则降低一级；友方非英雄随从的杀戮可以阻止衰减，但不会提升浓度。低浓度无杀戮时无法继续降低，改为保持低浓度并随机分食一个友方非英雄东京喰种随从。英雄、中立单位和建筑不参与该规则。RC 面板复用通用种族运行时状态面板，并显示低/中/高状态卡图；卡图悬浮预览复用 `CardTexturePreviewController`。该种族的施法回合在 UI 中命名为“赫子解放”；`KagunePowerResolver` 在赫子解放时按当时的 RC 浓度生成不可净化的能力快照，持续经过敌方回合，并在该玩家下一个回合开始时移除。这样甲赫的护甲与反伤、鳞赫的吸血等防御性能力拥有完整响应窗口，回合结束时的 RC 变化也不会重写已释放能力。尾赫获得移动攻击，高浓度额外移动力 +2；鳞赫在普通和高浓度均获得移动攻击，攻击 +1，高浓度改为 +2 并获得吸血；甲赫护甲 +1，高浓度改为 +2 并获得通用 `reflect`；羽赫获得每回合一次、不消耗主行动力的状态授予 `EffectAction`“羽针”，普通浓度造成 1 点伤害，高浓度造成 3 点。当前普通牌池随从覆盖四类赫子：黑山羊特工（甲赫、原生护甲 1）、安定区流浪者（4/5、鳞赫）、青铜树成员（3/5、羽赫、远程、飞行）、小丑临时工（6/12、尾赫）。2 阶英雄法术“蜈蚣形态”使用永久覆盖变身生成英雄衍生形态；该形态主动切断金木研卡牌身份，从而禁止继续使用金木研所属法术，死亡时恢复原形。2 阶升级牌“S阶喰种情报”是首张随从库牌，容量 1、冷却 2，从有限池不放回提供壁虎（8/8 鳞赫）、瓶兄弟（3/10 尾赫、攻速 2）、雾岛董香（6/6 羽赫、远程）和月山习（3/8 甲赫、护甲 2、嘲讽）；四者定义在本族 `tokens[]`，可被全局查卡但不进入公共牌池。卡牌静态字段 `armor` 表示单位原生护甲，进入 `CardState` 后作为非状态护甲基值；装备/手牌被动与状态护甲在其上叠加，净化仅移除状态部分，复生和新鲜状态初始化会恢复原生护甲。状态授予动作必须继续经过 `GrantedActionResolver` / `ActionRegistry`，从而让玩家、提示和 AI 共用合法性。规则说明升级牌“赫子之力”默认入手但无效果，只展示规则。表现使用 `TokyoGhoulAnimationProvider` 处理羽针、低浓度分食、蜈蚣形态和开启赫子解放时的全战场演出，持续赫子状态由 `CardStatusOverlay` 展示。全战场演出通过 `SpellAnimationRouter.register_board()` 注册；`GameManager` 只在扣费成功、规则状态已刷新后请求 `kagune_release`，演出期间锁定交互，AI 同样等待演出结束。
- 东京喰种英雄法术“尾赫齐发”使用通用 `apply_kagune_power` 效果，从 `KagunePowerResolver.create_kagune_payload()` 动态取得当前 RC 浓度下的尾赫能力；本回合攻击力 +1 由独立的可叠加状态提供，仍在本回合结束时移除；尾赫能力状态采用替换策略并复用通用赫子生命周期，持续至下个己方回合开始，避免多次施放重复叠加移动力。赫子持续表现按 `kagune_power` 状态标签识别，不依赖固定状态 id。英雄所属法术仍由 `HandPlayResolver.is_required_hero_on_board()` 与 `CardState.represents_card_id()` 统一校验；三种 `preserve_original_identity: false` 的金木研覆盖形态均不能施放该牌。
- SSS 阶随从“芳村功善”为 4/12：主动作“免单”使用通用 `friendly_minions_by_faction` 目标集合恢复全部己方东京喰种，并通过 `set_faction_runtime_state` 把 RC 浓度置为高；副动作“赫者化”每个单位实例仅可使用一次，永久覆盖变身为 4/10、巨兽、攻城 4 的“不杀之枭”。“高槻泉”为 5/10、瞬移、移动攻击，也可一次性覆盖变身为 12/12 巨兽“独眼之枭”。金木研三种覆盖形态、不杀之枭和独眼之枭均提供零主行动力副动作“恢复原形”，通过 `restore_transform` 主动还原；恢复后该动作随形态消失，原单位的赫者化一生一次消耗仍然保留。两种枭形态被击败时也会按覆盖变身规则恢复各自原形。“旧多二福”为 8/8，使用 `frontal_width_5` 同时攻击面前五格，并具有 `magic_immune` 与 `ranged_attack_immune`。“死堪”为 6/14，同时拥有尾赫、鳞赫、甲赫与羽赫；`KagunePowerResolver` 会收集并合并多个赫子 payload，因此赫子解放期间四种能力同时生效，不需要死堪专用分支。`TokyoGhoulAnimationProvider` 负责免单、赫者化与恢复原形演出。
