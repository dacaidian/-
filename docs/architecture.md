...
- 共生体：已接入 `symbiote` 种族块、`2/20` 的 1 阶英雄 `venom`（毒液）、1 阶远程随从共生神盾局探员、2 阶远程随从生物研究员、2 阶巨兽异噬体和3阶基因战士。毒液“自我剥离”和研究员“人工剥离”都使用 `play_animation -> damage -> add_card_to_hand` 配置链，固定造成2点非强化伤害并获得本族 `tokens[]` 中的无效果衍生法术 `symbiote_tissue`；人工剥离的第一个表现效果以 `owner_card_by_id: venom` 作为可执行性门闩，因此己方毒液不在场时整个无目标施法动作不会出现。共生体组织当前只用于占位，不可打出也不进入普通牌池。后续应把“有限子代池”“宿主附着/脱离”“子代进化”和“纳尔解放”拆成独立规则边界，避免在毒液英雄卡或 UI 中按具体子代写死。

共生体剥离表现采用“语义键 -> `SymbioteAnimationProvider` -> `SymbioteSeveranceVisual`”边界。自我剥离以黑红活体膜、湿润高光、伤口收缩和组织核分离为主体；人工剥离复用组织材质，但增加冷青固定器、牵引膜和标本封存囊。画布继承 `ThrottledProgressVisual` 并限制为24 FPS，复杂组织连接使用共享安全 Ribbon，不在规则层查询目标或生成手牌。修改后运行 `tools/test_symbiote.gd`、卡牌校验、图鉴测试和整项目检查。
