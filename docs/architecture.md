...
- `AttackAction`：普通攻击，包含远程规则、巨兽范围伤害、固定溅射、占领提示、护甲减伤、隐身目标过滤、攻击后破隐。
...
- SSS 阶随从“芳村功善”为 4/12：主动作“免单”使用通用 `friendly_minions_by_faction` 目标集合恢复全部己方东京喰种，并通过 `set_faction_runtime_state` 把 RC 浓度置为高；副动作“赫者化”每个单位实例仅可使用一次，永久覆盖变身为 4/10、巨兽、攻城 4 的“不杀之枭”。“高槻泉”为 5/10、瞬移、移动攻击，也可一次性覆盖变身为 8/8 巨兽“独眼之枭”。两种枭形态被击败时均按覆盖变身规则恢复各自原形，且一次性动作消耗不会因恢复原形而重置。`TokyoGhoulAnimationProvider` 负责免单与赫者化演出。
...
- 东京喰种：RC 浓度、四类赫子解放、金木研三种覆盖变身、S 阶与 SSS 阶有限随从库已接入；芳村功善与高槻泉及其赫者形态已经完成，旧多二福、死堪仍为后续能力设计占位。
