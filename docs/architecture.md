...
框体按实际承载密度分为五档，并在全局使用同一语义，而不是只服务手牌区：
...
皮肤母版通过 `tools/build_ui_skin_assets.py` 构建为运行时资源。生成母版可以保留宽阔装饰边缘，但运行时应通过九宫格预处理压缩装饰边带，并从同一视觉母版派生 `DRAWER`、`INSET`、`SECTION` 和 `HUD` 纹理；不能把大型概念框整体缩放后塞进紧凑控件，否则边框会侵占中心内容。横向按钮的四态必须来自同一轮廓，避免悬浮时布局跳动；按下态只通过图像反馈和内容轻微下移表达；手牌抽屉侧边按钮使用独立纵向 tab 资源，不能旋转横向按钮或强行拉伸文字。`PanelContainer` 由 StyleBox 自动落实安全区；普通 `Panel` 不会据此布局子节点，因此手牌抽屉等普通 Panel 必须在 controller 中显式维护与框体匹配的内容矩形。新增或重建 PNG 后，先运行 `tools/run_godot_validation.ps1 -ImportAssets`，再运行 `tools/test_ui_skin.gd` 验证纹理尺寸、状态差异、安全内容区、场景接入和九宫格契约。
