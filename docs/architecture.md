...
棋盘规则尺寸与棋盘视图尺寸必须分离。`CardBoard` 始终保留按逻辑索引排序的 49 个槽位、地面卡和空中卡；默认紧凑视图只隐藏外环并让 `GridContainer` 将中央 25 格按 5x5 重新排布，因此卡牌可以使用同一块物理战场空间放大显示。“完整战场”开关恢复 7x7 排布，但不得销毁节点、重绑 `CardState`、重排槽位索引或修改 `BoardCell` 能力。`BoardViewToggleController` 只负责开关样式、战场顶部中央定位、忙碌状态保护和视图同步；规则层不得读取当前视图来决定移动、攻击、补牌或目标合法性。

位置型表现必须使用当前可见节点的实时全局矩形。切换视图前清理尚未确认的目标选择，动作动画期间拒绝手动切换；矢量与方向选择器只为当前可见格建立命中区和预览。部署、移动、交换、普通攻击和显式目标法术若涉及隐藏外环，应先展开完整视图并等待容器完成一次布局，再采集动画坐标。群体、状态、路径和持续区域表现只合并 `is_visible_in_tree()` 的槽位矩形，禁止用隐藏外环保留的旧坐标绘制到全屏表现根节点。修改该边界后运行 `tools/test_board_view_modes.gd`、`tools/test_board_persistent_visuals.gd` 与完整项目检查。
...
- `SpellAction`：棋盘单位的施法动作。目标由 `SpellTargetResolver` 解析；施法后默认破除隐身，除非配置 `breaks_stealth: false`。动作开始时必须固化施法者 owner，并把同一份快照写入每个子效果的运行时 `effect_owner_id`，保证自伤致死、变身或其他中途清空来源的效果链仍能把后续入手、资源和死亡归属交给原施法玩家。需要自行编排多段表现时配置 `effect_handles_animation: true`，再用 `play_animation` 明确插入时序，避免动作层重复播放通用施法动画。
...
装备被动使用 `trigger: "while_equipped"`。属性型装备被动由 `HandPassiveResolver` 刷新；施法能力改写型装备也使用 `modify_spell_ability` / `modify_hand_spell_effects`，由 `HandSpellModifierResolver` 从已装备区读取。`HandPassiveResolver` 的刷新流程分成两步：先从手牌、装备区与正面场上来源收集一次 active passive effect snapshot，再把同一批配置应用到翻牌上限、种族技能、单位移动力、关键字、攻击、护甲、攻速、骑乘攻击和周期光环。场上应用范围统一走 owner 的 face-up minion 集合，覆盖地面层和飞行层，避免每个刷新器重复判断。`faction_id` 表示卡包与玩家阵营，不等价于生物类别；需要跨卡牌身份筛选“机械”“共生体”等单位集合时使用 `CardData.unit_traits` 与效果字段 `target_unit_traits`，不要用阵营筛选再维护排除名单。不要把装备属性逻辑写进 UI 或 `PlayerState.equip_card()`，除非只是区域 bookkeeping。
...
- 狐妖仙：尾数、献祭种族技能、默认入手升级、魅惑和复生体系。尾数规则真值保存在 `PlayerState.faction_resource_values`；`FactionSkillPanelController` 只读取它并构造专属九槽 `TailResourceMeter`。普通献祭先播放魂魄抽取，再使目标实际死亡并增加尾数；九尾生效后仍然杀死目标，但不再增加尾数，而由九尾升级的 before-target 效果给生效范围内单位授予复生。祸国使用同一条逐单位献祭链，并在每个目标前重新读取九尾修正，因此可以在一次结算中先增长至九尾，再让后续牺牲触发复生收益。
...
- “共生体组织”使用手牌法术通用字段 `target_card_ids` 和目标规则 `friendly_minions_by_card_ids`，只允许选择己方共生神盾局探员、生物研究员或基因战士。`HandSpellModifierResolver` 将卡牌筛选随解析后法术一起传递，`HandPlayResolver` 与 `SpellTargetResolver` 共用同一结果，避免 UI 可选目标和实际效果分叉。`AttachSymbioteOffspringEffect` 在目标再次校验通过后才调用玩家池抽取：`with_replacement_card_ids` 中的共生体战士保持在池中，其余结果立即移除。目标随后通过 `CardState.transform_to_card_data()` 执行不可逆永久进化，旧状态和旧卡牌能力全部清空，不创建 transform 状态，不保留宿主身份，但保留本回合已经消耗的行动经济。效果配置的 `inherit_host_base_stats_card_ids` 声明特殊遗传宿主；当前基因战士会在替换前读取其 `CardData.attack/health`，并在替换后通过 `add_permanent_attack()` / `add_permanent_max_health()` 将这组基础属性叠加到子代、写入 `permanent_stat_overrides`，生命上限增长同步增加当前生命。该加成不是状态且不可驱散。完成替换和遗传加成后必须刷新新旧控制者的持续被动，让纳尔等实时光环作为最终属性层重新应用；刷新不得提前到永久属性写入之前。抽取、卡牌替换、遗传加成和池消耗均在同一同步结算段完成，不允许动画层预抽或回滚池内容。
- 场上来源被动与手牌改写使用相同的快照边界。`HandPassiveResolver.collect_active_passive_effects(player, game_manager)` 汇总手牌、装备和己方正面场上单位的 `while_on_board` 效果。`target_faction_id` 用于真正覆盖整个阵营的效果；纳尔攻击光环使用 `target_unit_traits: ["symbiote"]`，只命中毒液及实际共生体子代，不包含同阵营宿主人类、异噬体和纳尔自身。`HandSpellModifierResolver.resolve_hand_spell()` 必须接收 `GameManager` 才能收集场上法术改写器；`HandPlayResolver` 的可用性、目标高亮、实际结算与 AI 评估都必须消费同一份动态解析结果。纳尔·解放使用该机制把“共生体组织”改为 `non_hero_minions`，并通过 `allow_any_non_hero_minion` 明确关闭固定宿主限制；效果层仍在抽池前二次校验目标。
...
池解锁、屠杀阈值、附着完成和吞噬完成等动画必须由拥有规则真值的 effect 在确认实际变化后发布，不得由动画层根据卡名或当前画面猜测。群体纳尔光环的表现目标可以通过通用 `play_animation.target_unit_traits` 过滤，但该字段只筛选已解析的可见矩形，不参与被动规则；新附着子代在完成永久属性和被动刷新后，若光环真实存在，再播放单目标受益反馈。持续状态由 `CardStatusOverlay` 消费真实状态：撕咬、恐惧、纳尔光环、吞噬继承层和沉默光环都使用一次几何绘制加合成透明度呼吸，不允许为每张卡逐帧重建路径。一次性程序化画布继承 `ThrottledProgressVisual` 并限制为24 FPS；复杂组织连接使用共享安全 Ribbon，柔光体调用方跳过不可见尺寸，避免把子像素退化几何提交给 Canvas 三角化器。修改后依次运行 `tools/test_symbiote.gd`、`tools/test_symbiote_offspring_pool.gd`、`tools/test_animation_routing.gd`、`tools/test_vfx_canvas_toolkit.gd`、`tools/test_vfx_redraw_scheduling.gd`、卡牌校验、图鉴测试和整项目检查。
...
`RightSideHudLayoutController` 只负责排列已经创建好的右侧 HUD 面板，例如回合状态、种族技能、种族时间和装备展示。列宽根据视口在 296px 至 340px 之间变化；每个框体的高度必须始终等于自身内容、皮肤安全边距和内容间距共同形成的自然高度，不得根据剩余屏幕高度单独拉伸。高度不足时可以压缩面板间距，但禁止互相覆盖或改变内容高度。它不决定面板是否可见，也不读取或修改玩法状态；`GameManager` 只把需要参与布局的 panel 列表交给它。

`RightSideHudStyle` 提供右侧面板共享的背景、边框、标题、按钮、指标块和资源刻度样式；`HudSymbolIcon` 提供不依赖种族素材的轻量矢量图标。面板标题和“法力、翻牌、资源分、装备类型、种族状态、种族技能”等稳定语义优先使用图标与 tooltip，精确玩法数值仍保留短格式。通用的小上限资源优先用离散刻度，较大资源使用进度条；如果资源本身拥有不可替代的形状语义，可以提供只读专属仪表，但必须继续嵌入统一 HUD 外壳并由 controller 传入前后数值。`TailResourceMeter` 是首个例子：九个尾槽、中央数字和 3/6/9 阈值属于表现，尾数增减仍只由玩家状态决定。`FactionSkillPanelController` 为玩家、资源值和技能可用性建立视图签名，签名未变化时必须复用现有节点，不能因无关的玩家状态信号清空并重建整棵面板。各 controller 可以选择自己的强调色，但不得复制一套新的面板外壳、宽度或定位常量。结束回合等对局主命令通过 `ApplicationUiStyle.style_match_action_button()` 使用统一主按钮四态，不允许保留场景默认按钮外观。
...
狐妖仙一次性表现采用“稳定路由门面 + 三个语义画布”的结构。`FoxSpiritAnimationProvider` 只注册 key、换算局部坐标、选择 HUD/手牌/棋盘锚点、统一时长并回收节点；`FoxSpiritTargetVisual` 负责献祭、九尾献祭、复生、勾魄、魅惑、狐念之术和九尾之尾入场；`FoxSpiritAreaVisual` 只负责 2x2 狐火的准确边界、四角弧线落火与逐格灼烧；`FoxSpiritRitualVisual` 负责祸国、九尾军势生成和多目标天狐化。统一语言是绯红/暗紫丝状妖气、冷白狐眼、魂魄丝线、镜面薄膜、蓝紫狐火和珍珠金九尾边缘，不使用粉色爱心、普通橙色火焰或无语义光圈。狐尾与魂丝必须是带镜面高光、柔边和宽度变化的实体妖气，目标周围使用低遮挡香雾膜和错相光点建立纵深；狐火区域以低矮妖雾、灵火体积和准确边界共同表达，禁止仅画发光折线。

语义 key 按上下文声明：`sacrifice`、`nine_tail_sacrifice`、`fox_reborn`、`soul_hook`、`charm`、`fox_mind_art`、`nine_tail_tail_enter` 属于目标/矩形表现，`foxfire` 属于 area，`ruin_country` 属于矩形仪式，`nine_tail_army` 属于 board，`ruin_country_targets` 与 `celestial_fox_evolve` 属于 `multi_rect`。普通献祭的魂魄轨迹优先指向当前可见 `TailResourceMeter`；九尾献祭改为月白复生语义并指向苏妲己，不伪造已经封顶的尾数增长。九尾的 `grant_reborn` 显式配置 `fox_reborn`，通用复生结算仍使用 `reborn` 默认表现，避免巨水元素等其他种族复生错误显示狐尾。祸国先播放全场预告，再由效果层把已经解析的友方非英雄目标集合交给 `ruin_country_targets` 统一标记，禁止用抽象棋盘格伪造目标；随后沿真实规则逐个献祭，最后只播放一次九枚狐尾种子飞向手牌的军势反馈，不能为九张生成牌重复九次大型全场演出。

持续表现继续读取真实数据。`TailResourceMeter` 只缓存前值和当前值用于过渡，不拥有资源；1-2、3-5、6-8、9 尾分别使用暗红、胭脂紫、月白紫缘和珍珠金阶段色，并同时显示中央数字与 3/6/9 阈值。九条尾槽只在数值变化期间以24 FPS播放错相增长，稳定后停止 `_process()` 和几何重建；`FactionSkillPanelController` 通过视图签名复用未变化的整棵种族 HUD，不能因无关玩家状态信号反复销毁仪表。普通献祭在魂魄进入仪表后，再沿同一临时根节点向场上苏妲己回勾一缕妖气；Provider 返回后规则层才更新尾数、被动与攻击数字，从而形成连续反馈而不让 UI 提前修改数值。勾魄、永久魅惑和狐念之术只在状态变化时重建几何，持续呼吸改由 `self_modulate` Tween 完成；状态移除后覆盖和 Tween 必须立即停止，施法动画不能承担控制权生命周期。

进化、复生授予、手牌生成和衍生随从入场的表现入口是通用配置钩子，不属于狐妖专用规则：`EvolveUnitsEffect.animation` 在替换前请求 `multi_rect`，`GrantRebornEffect.animation` 覆盖默认 `reborn` 授予反馈，`AddCardToHandEffect.animation` 在批次生成前请求 board 表现，随从 `CardData.animation` 在卡牌正面放入地面/飞行层后、入场触发前请求 slot 表现。效果类只转发配置 key，不识别天狐、九尾、祸国或九尾之尾的卡牌 id。新增或调整狐妖仙表现后运行 `tools/test_fox_spirit_animation_provider.gd`、`tools/test_animation_routing.gd`、`tools/test_right_side_hud.gd` 与卡牌数据校验。
...
野兽人的表现采用“稳定门面、语义绘制器、共享色彩”结构。`BeastmenAnimationProvider` 只声明路由、换算全局/局部坐标、选择棋盘与手牌锚点、控制三段式时长并回收节点；`BeastmenCombatVisual` 负责同系进化、卡扎克杀戮成长、万魔充能命中和单体咆哮受益；`BeastmenRitualVisual` 负责野蛮咆哮、野性呼唤、兽径起手、废灭仪式和混沌腐蚀全场结算；`BeastmenPathVisual` 只消费有序格子矩形并绘制连续挖掘；`BeastmenVfxPalette` 统一深土、干涸血红、兽骨、铜锈、暗橙与少量混沌紫绿。规则 resolver 不得实例化这些节点，也不得根据视觉需要改写目标、伤害、进化或充能结果。

野兽人视觉强调物理重量和群体压迫：`beastmen_evolution` 使用血脉回收、兽骨角冠、脊刺生长与低矮地裂表达卡牌已被永久替换；`beastmen_slaughter` 使用闭合兽颚、干血爪痕和混沌核心表达卡扎克的不可驱散永久成长，但仍保留通用近战撞击与真实死亡反馈；`savage_roar` 是来源卡牌的一次全场战吼风压，`savage_roar_buff` 是每个友军攻击区附近的短促战纹，两者必须分 key，避免群体施加状态时重复播放完整咆哮；`wild_call` 用骨哨、蹄印与兽皮卡印表达荒野单位进入手牌；`wanmo_ritual` 用石岩裂纹、充能内收、巨兽影和送往手牌的角兽印记表达按真实层数生成古尔兽。所有短演出控制在普通动作 0.5 至 0.95 秒、中型法术 0.8 至 1.1 秒、废灭仪式约 1.4 秒，不使用单字 Label、通用圆形光圈或遮挡卡面的大片烟雾。

表现归属必须与规则归属一致。`chaos_corruption_burst` 通过 `GameManager.play_board_effect_animation()` 进入 board 路由，在棋盘范围内绘制贴地腐蚀脉络、土壤暗化与低矮冲击波；`beast_path` 通过 `play_path_effect_animation()` 进入 path 路由，按五个真实矩形连续绘制沟壑、碎土、蹄印和端点骨桩。兽径权威状态保存在 `BoardCell.beast_path_ids`，奥术空间交换后随单元格移动；`CardState.has_beast_path` 与 `CardStatusOverlay` 仅是显示镜像。混沌腐蚀数值、万魔岩充能数量、兽径入口和野蛮咆哮临时战纹均使用贴边低遮挡常驻视觉，并只在真实状态存在时启用 30 FPS 刷新。调整后运行 `tools/test_beastmen_animation_provider.gd`、`tools/test_animation_routing.gd` 与 `python tools/validate_cards.py`。

影月议会表现采用“稳定路由门面、目标法术画布、仪式画布、共享色彩”四层结构。`ShadowmoonAnimationProvider` 只声明路由、寻找古尔丹/棋盘/手牌等语义锚点、换算坐标并管理时长与节点回收；`ShadowmoonTargetVisual` 负责自伤、邪能传输、法力燃烧、撕咬、灵魂虹吸、诅咒、过载爆裂和单体疯狂响应；`ShadowmoonRitualVisual` 负责邪能狂乱广播、恶魔召唤、黑暗之门、地狱火献祭、周期低语和多目标响应；`ShadowmoonVfxPalette` 统一酸性黄绿核心、墨绿流体、炭黑边缘、黑紫空间膜、暗红诅咒和地狱火暗橙。邪能主体使用黑紫膜包裹的黏稠高压核心、错相鼓胀和酸性内亮，灵魂与诅咒分别偏蓝紫和暗红紫；恶魔裂隙必须有空间膜厚度，疯狂广播必须是低矮压力前沿。绿色线条、绿色空心圆和全卡绿色闪烁只能作次级提示，不能再作为主体。规则层不得直接实例化这些节点，也不得按表现需要改变伤害、目标、周期或死亡顺序。

需要严格表现结算顺序的效果链使用通用 `play_animation` 效果作为纯表现步骤，并通过 `presentation_scope` 声明 `target`、`source_to_target`、`multi` 或 `board` 上下文。邪能灌注与邪能过载固定为“古尔丹自伤表现并扣血 -> 邪能从来源传输到目标 -> 状态落定 -> 成功邪能施法广播疯狂”；配置中的效果顺序就是权威顺序，Provider 不重新解释规则。`ApplyStatusEffect`、`PeriodicStatusAuraEffect` 和 `DamageEffect` 在 `multi` 模式下先确定并完成全部规则目标，再请求一次共享多目标演出；路由不存在时才回退到逐目标局部动画，避免目标越多就重复播放整段仪式。

`after_spell_cast` 若需要“整组效果每回合只触发一次”，应在同组效果上配置相同的 `once_per_turn_group`。`SpellCastTriggerResolver` 会先收集一张升级牌中本次匹配的全部效果，再向 `TurnEventLedger` 申领该玩家的组；首次申领会放行整组并保持原顺序，后续申领会跳过整组，`begin_turn()` 在全局回合切换时统一清空。目前该字段只属于 `after_spell_cast` 入口，扩展到其他事件前必须先让对应 resolver 接入同一账本，并补齐验证。该闸门属于事件时序，不应用状态 `stack_policy` 代替：状态可能因不同施法来源形成不同堆叠键，而且纯表现步骤仍会被重复播放。
...
持续状态只消费真实状态：邪能灌注把低亮邪能脉络收束到攻击区；邪能过载显示压力核心和炭化裂纹；六类疯狂各自显示颚、灵魂涡流、爪痕、恶魔角或低语眼；基尔加丹低语使用暗红紫恶魔眼；地狱火来源的火焰状态使用贴底邪能火与剩余回合刻度。它们全部由 `CardStatusOverlay` 管理，状态移除后立即停止刷新。灌注、过载、疯狂、伤害加深和低语只在状态变化时构建一次轻量路径，之后以不触发 `_draw()` 的 `self_modulate` Tween 保留呼吸；确实改变火舌形态的地狱火按12 FPS重绘，瞬时破盾和解除演出仍保持30 FPS。卡面循环不得使用高分段焦点 Ribbon。修改后运行 `tools/test_vfx_canvas_toolkit.gd`、`tools/test_vfx_redraw_scheduling.gd`、`tools/test_shadowmoon_animation_provider.gd`、`tools/test_animation_routing.gd`、`python tools/validate_cards.py` 和安全整项目检查。
...
- 共生体：基础种族、组织剥离、玩家级子代池、十三张池内/衍生随从、共生体组织附着和首批子代能力已经接入；更复杂的宿主能力、子代组合进化以及纳尔的后期全局强化仍属于后续机制设计。
