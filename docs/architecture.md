...
棋盘规则尺寸与棋盘视图尺寸必须分离。`CardBoard` 始终保留按逻辑索引排序的 49 个槽位、地面卡和空中卡；默认紧凑视图只隐藏外环并让 `GridContainer` 将中央 25 格按 5x5 重新排布，因此卡牌可以使用同一块物理战场空间放大显示。“完整战场”开关恢复 7x7 排布，但不得销毁节点、重绑 `CardState`、重排槽位索引或修改 `BoardCell` 能力。`BoardViewToggleController` 只负责开关样式、位置、忙碌状态保护和视图同步；规则层不得读取当前视图来决定移动、攻击、补牌或目标合法性。
