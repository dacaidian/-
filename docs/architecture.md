...
- 影月议会：已接入英雄古尔丹、默认入手升级牌“邪能狂乱”和基础随从“混乱兽人”。邪能体系使用法术/施法动作的 `spell_tags` 标记，例如 `spell_tags: ["fel"]`；手牌区升级牌通过 `trigger: "after_spell_cast"`、`active_zone: "hand"` 和 `required_spell_tags` 监听成功施法。古尔丹的“邪能灌注”是首个邪能施法动作：目标规则为 `friendly_non_hero_minions`，对古尔丹自身造成固定 2 点自伤，并给选中的友方非英雄随从附加本回合 `fel_infusion` 攻击状态，同时触发邪能。混乱兽人的疯狂增益由“邪能狂乱”给 `friendly_minions_by_card_ids` 附加临时攻击状态实现，不写死在随从类里。后续恶魔召唤、黑暗之门和更多疯狂分支应继续沿用 spell tag + hand trigger，而不是在 `SpellAction` 或 `GameManager` 中按卡牌 id 分支。
