...
const TARGET_RULE_PLAYER_OWNED_NON_HERO_MINIONS := "player_owned_non_hero_minions"
const TARGET_RULE_ENEMY_MINIONS := "enemy_minions"
const TARGET_RULE_ENEMY_NON_HERO_MINIONS := "enemy_non_hero_minions"
...
static func can_target(
	...
	match target_rule:
		...
		TARGET_RULE_PLAYER_OWNED_NON_HERO_MINIONS:
			return target.is_minion() and not target.is_hero() and target.owner_id != ""
		...
		TARGET_RULE_ENEMY_NON_HERO_MINIONS:
			return (
				target.is_minion()
				and not target.is_hero()
				and resolved_source_owner_id != ""
