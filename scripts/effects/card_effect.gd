...
func get_target_states(source_state: CardState, effect_data: Dictionary, game_manager: Node) -> Array[CardState]:
	...
	match target:
		...
		EffectData.TARGET_ADJACENT_ENEMY_MINIONS:
		...
		EffectData.TARGET_FRIENDLY_MINIONS_BY_FACTION:
...
func get_adjacent_enemy_minions(source_state: CardState, game_manager: Node, include_heroes: bool) -> Array[CardState]:
	var targets: Array[CardState] = []
	if source_state == null or game_manager == null:
		return targets
	if source_state.owner_id == "":
		return targets
	...
	for slot_index in adjacent_slots:
		for target_state in game_manager.get_board_states_at_slot(slot_index):
			if not BoardQuery.is_face_up_minion(target_state):
				continue
			if not include_heroes and target_state.is_hero():
				continue
...
func get_friendly_minions(source_state: CardState, effect_data: Dictionary, game_manager: Node) -> Array[CardState]:
	var targets: Array[CardState] = []
...
func get_friendly_minions_by_faction(source_state: CardState, effect_data: Dictionary, game_manager: Node) -> Array[CardState]:
	var targets: Array[CardState] = []
	var faction_id := str(effect_data.get(EffectData.KEY_TARGET_FACTION_ID, ""))
	if faction_id == "" and source_state != null and source_state.data != null:
		faction_id = source_state.data.faction_id
	if faction_id == "":
		return targets

	for target_state in get_friendly_minions(source_state, effect_data, game_manager):
		if target_state.data != null and target_state.data.faction_id == faction_id:
			targets.append(target_state)
