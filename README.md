...
- **自适应手牌抽屉**：法术、随从、升级、装备四区保留独立纵向滚动，横向滚动被禁用；抽屉尺寸始终限制在可见视口内，空区自动收缩，非空区按真实可用宽度换行并按卡牌行数共享净可用高度，焦点切换不会改变布局或重置滚动位置。
