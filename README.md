...
- **UI 皮肤集中管理**：`GameUiSkin` 只管理位图、九宫格边距、安全内容区和控件状态；`ApplicationUiStyle` 负责主菜单、图鉴等应用页面的语义样式；`RightSideHudStyle` 负责战斗 HUD 的强调色和紧凑控件。页面 controller 只选择语义变体，不自行复制大型代码绘制外框，也不能把内容放进木框、金属角或内侧斜面覆盖范围。
